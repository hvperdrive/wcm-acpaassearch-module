...
var PermissionsSecurity = require("app/helpers/modules/lib").PermissionsSecurity("members");
// Building the baseUrl based on the configuration. Every API call needs to be located after the api/ route
var baseUrl = "/" + config.api.prefix + config.api.version + "members";

module.exports = function(app, hooks) {

    // Management routes
    app.route(baseUrl).get(ProfileSecurity, MethodSecurity.read, PermissionsSecurity, membersController.read);

    app.route(baseUrl + "/:uuid").get(ProfileSecurity, MethodSecurity.read, PermissionsSecurity, membersController.readOne);

    // app.route(baseUrl).post(ProfileSecurity, MethodSecurity, PermissionsSecurity, membersController.create);

    app.route(baseUrl + "/:uuid").put(ProfileSecurity, MethodSecurity.update, PermissionsSecurity, membersController.update);

    app.route(baseUrl + "/:uuid").delete(ProfileSecurity, MethodSecurity.delete, PermissionsSecurity, membersController.remove);
